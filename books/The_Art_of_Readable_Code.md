...
- 减少变量：减少不能改进可读性的变量，会是代码更精炼且同样容易理解
  - 移除没有价值的临时变量（中间结果）
  - 减少中间结果：速战速决通常是一个好策略
  - 减少控制流变量：
    - 控制流变量通常可以通过更好地运用结构化编程而消除（如，从循环跳出break）
    - 如果有多个嵌套循环，没有办法通过简单的break退出，解决方案通常是把移到一个新函数中（要么循环中的代码，要么整个循环）
- 缩小变量作用域，越小越好：
  - 让你的变量对尽量少的代码行可见：让所有变量都“缩小作用域”是一个好主意，不仅是“避免全局变量”
  - 通常越严格的访问控制越好：模块、类、函数以及语句块作用域
    - 因为这样有效地减少了读者同时需要考虑的变量个数
    - 例：一个类的两个方法间需要共享一个变量，应该局部变量，通过参数传递到另一个方法（而不是使用一个成员变量）
    - 例：if 语句的判断条件为一个返回布尔值的方法，应该将方法调用放到if语句块上（而不是在语句块外赋值给变量，再if语句中使用变量），这样减小了这个方法返回值的作用域
  - 类成员访问“尽量使方法变成静态的”：静态方法是让读者知道“这几行代码与那些变量无关”的好办法
  - 数据分离：将大的类拆分成互相独立的小一些的类，把大文件拆分为小文件，将大函数拆分成小函数
- 把定义向下移：减少读者心智负担，避免强迫读者马上思考变量
  - 原来的C语言要求把所以变量定义在函数或语句块的顶端，C99和C++去掉了这个要求
- 只写一次的变量更好，使得代码更容易理解：
  - “永久固定”的变量更容易思考
  - 常量往往不会引来麻烦
...
> - 工程学: 把大问题拆分成小问题再把这些问题的解决方案放回在一起; 把工程学原则应用于代码会使代码更健壮并且更易读
> - "把一般代码和项目专有的代码分开”, ”积极地发现和抽取不相关的子问题“
...
- 不相关子问题示例：
  - 纯工具代码: 抽取出来放到一个独立工具库, 可以复用
  - 其他多用途代码：如，美化输出
- 抽取的为独立函数或库的好处：
  - 使用代码更简单
  - 更容易改进：添加功能、改进可读性、处理边界情况等
  - 可以独立测试
- 创建大量通用代码: 它完全地从项目的其他部分解耦出来：易于开发、测试、理解
- 编程风格(大多数变成都包括了两者的组合)：
  - 自顶向下：先设计高层次模块和函数，然后根据支持它们的需要来实现低层次函数
  - 自底向上：尝试首先预料和解决所有子问题，然后用这些代码段来建立更高层次的组件
- 项目专有的功能：
  - 理想情况下，你所抽取的子问题对项目一无所知，但是就算不是这样，分离子问题仍然可能创造奇迹
- 简化已有接口
  - 人人都爱提供整洁接口的库，它让你的代码看起来优雅、简单而又强大
    - 那种参数少，不需要很多设置并且通常只需要花一点工夫就可以使用的库
  - 如果你所用的接口并不整洁，你可以创建自己整洁的“包装”函数
  - 你永远都不要安于使用不理想的接口
    - 你总是可以创建你自己的包装函数来隐藏接口的粗陋细节，让它不再成为你的阻碍
- 按需重塑接口
  - 程序中很多代码在那里只是为了支持其他代码，这些“粘附”代码常常和程序的实际逻辑没有任何关系，这种传统的代码是抽取到独立函数的最好机会
- 过犹不及
  - ”积极“是相对的，引入过多小函数实际上对可读性是不利的，每一个函数存在一个小的（却有形的）可读性代价

### 第十一章：一次只做一件事

> 应该把代码组织得一次只做一件事情, 同时做几件事的代码很难理解

- 流程：
  - 列出代码所做的所有“任务”，“任务”没有严格定义：如，“确保这个对象有效” 或 “遍历树中的所有结点"
  - 尽量把不同任务拆分到不同函数中，或者至少是代码中不同的段落中

### 第十二章：把想法变成代码

> 如果你不能把一件事情解释给你的祖母听的话说明你还没有真正理解它

- 当把一件复杂的事向别人解释时，那些小细节很容易就会让他们迷惑
- 把一个想法用“自然语言”解释是个很有价值的能力，可以帮助他人理解，也能帮助你自己把想法想得更清晰
- 用“自然语言”描述程序然后用这个描述来帮助你写出更自然的代码
- “小黄鸭调试法/橡皮鸭技术”：仅仅通过大声把问题描述出来，往往就能帮助找到解决办法
...
- 从项目中消除不必要的功能，不要过度设计
  - 程序员倾向于高估有多少功能真的对于他们的项目来讲是必不可少的
  - 程序员还倾向于低估实现一个功能所要花的工夫
    - 我们乐观地估计了实现一个粗糙原型所要花的时间，但是忘记了在将来代码库的维护、文件以及后增的“重量”所带来的额外时间
- 重新考虑需求，解决版本最简单的问题，只要能完成工作就行
  - 不是所有的程序都需要运行得快，100%精确，并且能处理所有的输入
  - 如果你真的仔细检查你的需求，有时你可以把它削减成一个简单的问题，只需要较少的代码
- 宇宙的自然法则：随着任何坐标系统的增长，把它粘合在一起所需要的复杂度增长得更快
- 让你的代码库越小，越轻量级越好：
  - 创建越多越好的“工具”代码来减少重复代码
  - 减少无用代码或者没有用的功能
  - 让你的项目保持分开的子项目状态
  - 总的来说，要小心代码的“重量”。让它保持又轻又灵
- 修剪掉碍事和没用的代码也是个好主意。不要心疼在上面所花的时间
- 经常性地阅读标准库中的整个API（所有函数/模块/类型的名字），保持对它们的熟悉程度
- 重用库或选择合适的工具：如，使用Unix工具来代替自己编写代码
